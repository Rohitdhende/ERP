...
const UnderGroupLocation = () => {
  ...
  const newCountryList = countries.filter((items) => {
    return items.continent.includes(formData?.continent);
  });

  //Update State list after selecting the country
  const newStateList = states.filter((items) => {
    return items.country.includes(formData?.country);
  });
      ...
      >
        ...
        >
          ...
          <Select
            ...
            disabled={formData.continent === '' ? true : false}
